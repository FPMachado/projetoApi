    ...
    <nav class="p-5 bg-black shadow md:flex md:items-center md:justify-between">
        ...
        <ul class="mt-0 md:flex md:items-center z-[-1] md:z-auto md:static absolute bg-black text-yellow-500 w-full left-0 md:w-auto md:py-0 py-4 md:pl-0 pl-7 md:opacity-100 opacity-0 top-[-400] transition-all ease-in duration-500">
            @if (!is_null(auth()->user()->email_verified_at))
                {{-- //TODO CRIAR FLAG ADMIN NA TABELA DE USUÁRIOS --}}
            ...
            @if (Auth::check())
                <div class="relative">
                    ...
                    <div id="dropDown" class="inline-block absolute bg-white border-gray-500 py-1 shadow-md rounded-md" style="width:137px; display: none; margin-left: -32px !important;">
                        <a class="block text-sm text-black px-3 py-1 bg-white hover:bg-yellow-500 duration-500" href="{{route('my-personal-list.movie.index', ['user' => auth()->user()->id])}}">Minha Lista</a>
                        <a class="block text-sm text-black px-3 py-1 bg-white hover:bg-yellow-500 duration-500" href="{{route('profile.index', ['user' => auth()->user()->id])}}">Meu Perfil</a>
